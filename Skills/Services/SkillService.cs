...
public class SkillService(IDbContextFactory<SkillsContext> factory)
{
    ...
    /// <summary>
    /// Imports into the DB the skills of the given stream according to the chosen ImportType.
    /// </summary>
    /// <param name="stream">The stream of the excel file</param>
    /// <param name="startCell">The Excel start cell of the header</param>
    /// <param name="importType">The <see cref="ImportType"/>, default <see cref="ImportType.Purge"/></param>
    /// <returns>The result of the treatment as <see cref="KeyValuePair"/> of an <see cref="ImportState"/> and a <see cref="string"/> (state, reason).</returns>
    public async Task<KeyValuePair<ImportState, string>> ImportXlsxAsync(Stream stream, string startCell, ImportType importType = ImportType.Purge)
    {
        ...
        await ms.DisposeAsync(); // If we dispose it earlier we won't be able to enumerate over the Excel file rows
    ...
    /// <summary>
    /// Deletes all skills from DB and imports given ones.
    /// </summary>
    /// <param name="skills">The skills to add to the app</param>
    /// <returns>The result of the treatment as <see cref="KeyValuePair"/> of an <see cref="ImportState"/> and a <see cref="string"/> (state, reason).</returns>
                ...
                else
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(row.SubCategory))
                    {
                        if (oldSubCategory != null)
                        {
                            ...
                            skill.SubCategoryId = oldSubCategory.Id;
                            skill.SubCategory = oldSubCategory.Value;
                        }
                        else
                        {
    ...
    /// <summary>
    /// Forcibly removes all skills from DB.
    /// </summary>
    ...
    /// <summary>
    /// Exports the skills of the given user.
    /// </summary>
    /// <param name="data">The skills to export</param>
    /// <param name="user">The user</param>
    /// <returns>A stream representing an Excel file.</returns>
    ...
    /// <summary>
    /// Imports the given skills of the given user.
    /// </summary>
    /// <param name="stream">The stream of the excel file</param>
    /// <param name="startCell">The Excel start cell of the headers row</param>
    ...
    public async Task<KeyValuePair<ImportState, string>> ImportUserSkillAsync(Stream stream, string startCell, UserModel user)
    {
        ...
        foreach (var row in rows)
        {
            ...
            AbstractSkillModel? skill = db.Skills.FirstOrDefault(x => x.Type == row.Type && x.Category == row.Category && x.SubCategory == row.SubCategory && x.Description == row.Description) ?? // skill to abstract
                                        db.SoftSkills.FirstOrDefault(x => x.Type == row.Type && x.Category == row.Category && x.SubCategory == row.SubCategory && x.Description == row.Description) // soft-skill to abstract
                                        as AbstractSkillModel;
            
            if (int.TryParse(row.Level, out var level))
            {
                if (level > 0)
                {
                    if (skill != null)
    ...
    /// <summary>
    /// Exports the given skills from DB (used in admin area).
    /// </summary>
    /// <param name="data">The given skills</param>
    /// <param name="levels">The given levels</param>
    /// <param name="softLevels">The given soft skills</param>
    /// <returns>A stream representing an Excel file.</returns>
    public async Task<Stream> ExportSkillsAsync(List<AbstractSkillModel> data, Dictionary<Guid, List<TypeLevel>> levels, Dictionary<Guid, List<SoftTypeLevel>> softLevels)
    {
        var values = new List<Dictionary<string, object>>();
        values.Add(new Dictionary<string, object>
        {
        ...
        foreach (var skill in data)
        {
            ...
            if (skill.Type?.ToLower() == "soft-skill")
            {
                ...
                lvl3 = levels.FirstOrDefault(x => x.Key == skill.Id).Value.FirstOrDefault(x => x.Level == 3)?.Value ?? string.Empty;
                lvl4 = levels.FirstOrDefault(x => x.Key == skill.Id).Value.FirstOrDefault(x => x.Level == 4)?.Value ?? string.Empty;
            }
            else
            {
                lvl1 = softLevels.FirstOrDefault(x => x.Key == skill.Id).Value.FirstOrDefault(x => x.Level == 1)?.Value ?? string.Empty;
                lvl2 = softLevels.FirstOrDefault(x => x.Key == skill.Id).Value.FirstOrDefault(x => x.Level == 2)?.Value ?? string.Empty;
                lvl3 = softLevels.FirstOrDefault(x => x.Key == skill.Id).Value.FirstOrDefault(x => x.Level == 3)?.Value ?? string.Empty;
                lvl4 = softLevels.FirstOrDefault(x => x.Key == skill.Id).Value.FirstOrDefault(x => x.Level == 4)?.Value ?? string.Empty;
            }
            ...
            row.Add("Column5", lvl1);
            row.Add("Column6", lvl2);
            row.Add("Column7", lvl3);
            row.Add("Column8", lvl4);
            values.Add(row);
        }
