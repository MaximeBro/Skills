...
public partial class AuthorizedComponent : FullComponentBase, IDisposable
{
    ...
    [Parameter] public string Roles { get; set; } = "Admin, Manager, User";
    [Parameter] public bool ShowDialog { get; set; }

    protected override async Task OnInitializedAsync()
    {
        AuthenticationService.OnNotAuthorized += ShowLoginDialogAsync;
        var authorized = await AuthenticationService.HasRequiredRoleAsync(AuthenticationState, Roles?.Split(", ") ?? []);
        if(!authorized)
        {
